...
window.dekkoModule = function (object) {
	...
	try {
		...
		if (typeof object.item.scroll == 'string' && object.item.scroll == 'fixed') {
			t.scrollBoolean = true;
			t.scrollBooleanO = true;
			t.appendOffsetTop = ($(object.append).offset().top - 20);
			$(window).on('scroll touchmove mousewheel', function(event) {
				t.scrollTop = $(this).scrollTop();
				if (t.scrollBoolean && t.scrollTop > t.appendOffsetTop) {
					t.wrap.css(css.wrapScroll);
					t.scrollBoolean = false;
					t.scrollBooleanO = true;
				}

				if (t.scrollBooleanO && t.scrollBoolean === false && t.scrollTop < t.appendOffsetTop) {
					t.wrap.css(css.wrapScrollStatic);
					t.scrollBooleanO = false;
					t.scrollBoolean = true;
				}
			});
		}
